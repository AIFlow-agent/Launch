...
const Sidebar = ({ show, children, side, className }: SidebarProps) => {
  return (
    <SidebarTransition show={show} side={side}>
      <nav
        className={clsx(
          "flex flex-1 flex-col overflow-x-hidden [background-color:rgba(255,255,255,0.26)] p-4 ring-1 ring-white/10",
          className
        )}
      >
        {children}
...
export const SidebarControlButton = ({
  ...
  return (
    <button
      className={clsx(
        "fixed z-20 m-1 rounded-md transition-all sm:m-2",
        side === "right" && "right-0"
      )}
      onClick={() => setShow(!show)}
    >
          <img 
            src="https://agent.aiflow.guru/expand.png" 
