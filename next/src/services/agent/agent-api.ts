...
type ApiProps = Pick<RequestBody, "model_settings" | "goal" | "tokenId"> & {
  session?: Session;
...
export class AgentApi {
  ...
  async createAgent(): Promise<void> {
    ...
    const agent = await this.props.agentUtils.createAgent({
      ...
      tokenId: this.props.tokenId!=undefined?this.props.tokenId:""
    });
    this.agentId = agent?.id;
    console.log("createAgent:agentId["+this.agentId+"]");
  }
