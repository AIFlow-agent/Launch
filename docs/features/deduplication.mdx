...
AIFlow automatically handles deduplicating data whenever your scrapers re-run.

## How It Works

When saving data, AIFlow uses a **unique key** (or composite key) based on the record's fields to determine if the data is new or if it is a duplicate of data that has already been saved.


| Scenario | Action Taken by AIFlow |
| --- | --- |
